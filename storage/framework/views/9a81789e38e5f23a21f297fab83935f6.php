<div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4 md:p-6">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-6 md:mb-8">
            <h1 class="text-2xl md:text-4xl font-bold text-slate-800">Admin Dashboard</h1>
            <p class="text-slate-500 mt-2 text-sm md:text-base">Manage modules, users, and enrollments</p>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 mb-6 md:mb-8">
            <div class="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-slate-500 text-xs md:text-sm font-medium">Total Modules</p>
                        <p class="text-2xl md:text-3xl font-bold text-slate-800 mt-1"><?php echo e($totalModules); ?></p>
                    </div>
            ...
            <div class="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-slate-500 text-xs md:text-sm font-medium">Total Users</p>
                        <p class="text-2xl md:text-3xl font-bold text-slate-800 mt-1"><?php echo e($totalUsers); ?></p>
                    </div>
            ...
            <div class="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-slate-500 text-xs md:text-sm font-medium">Active Enrollments</p>
                        <p class="text-2xl md:text-3xl font-bold text-slate-800 mt-1"><?php echo e($activeEnrollments); ?></p>
                    </div>
            ...
            <div class="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-slate-500 text-xs md:text-sm font-medium">Teachers</p>
                        <p class="text-2xl md:text-3xl font-bold text-slate-800 mt-1"><?php echo e($teachers->count()); ?></p>
                    </div>
        ...
        <div class="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6 mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4 md:mb-6">
                <div class="flex items-center mb-4 md:mb-0">
                    ...
                    <div>
                        <h2 class="text-lg md:text-xl font-bold text-slate-800">Manage Modules</h2>
                        <p class="text-slate-500 text-xs md:text-sm">Add, toggle, and manage course modules</p>
                    </div>
                </div>
            </div>

            <form wire:submit="addModule" class="mb-4 md:mb-6 flex flex-col md:flex-row gap-3 md:gap-4">
                <div class="flex-1">
                    <input wire:model="newModule" type="text" placeholder="Enter new module name..." class="w-full border border-slate-300 rounded-lg md:rounded-xl px-3 md:px-4 py-2.5 md:py-3 text-sm md:text-base focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
                </div>
                <button type="submit" class="bg-indigo-600 text-white px-4 md:px-6 py-2.5 md:py-3 rounded-lg md:rounded-xl hover:bg-indigo-700 transition shadow-sm font-medium flex items-center justify-center text-sm md:text-base">
                    <svg class="w-4 h-4 md:w-5 md:h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
                    Add Module
                </button>
            </form>
...
unset($__errorArgs, $__bag); ?><!--[if ENDBLOCK]><![endif]-->
            ...
            <!-- Search -->
            <div class="mb-4">
                <input wire:model.live.debounce.300ms="searchModules" type="text" placeholder="Search modules..." class="w-full md:w-64 border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
            </div>
                ...
                <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $modules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $module): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="border border-slate-200 rounded-xl p-4 bg-white">
                        <div class="flex items-start justify-between mb-3">
                            ...
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium <?php echo e($module->active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'); ?>">
                                <?php echo e($module->active ? 'Active' : 'Inactive'); ?>

                            </span>
                        </div>
                        <div class="text-sm text-slate-600 mb-2">
                            <span class="font-medium">Enrollments:</span> <?php echo e($module->enrollments()->where('status', 'enrolled')->count()); ?>/10
                        </div>
                        <div class="flex flex-wrap gap-1 mb-3">
                            ...
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                                <span class="text-slate-400 text-xs">No teachers assigned</span>
                            <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                        </div>
                        <button wire:click="toggleModule(<?php echo e($module->id); ?>)" class="w-full inline-flex items-center justify-center px-3 py-2 rounded-lg text-sm font-medium <?php echo e($module->active ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'); ?> transition">
                            <?php echo e($module->active ? 'Deactivate' : 'Activate'); ?>
        ...
        <div class="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6 mb-6">
            <div class="flex items-center mb-4 md:mb-6">
                <div class="w-8 h-8 md:w-10 md:h-10 bg-emerald-100 rounded-lg md:rounded-xl flex items-center justify-center mr-3 md:mr-4">
                    <svg class="w-4 h-4 md:w-5 md:h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                </div>
                <div>
                    <h2 class="text-lg md:text-xl font-bold text-slate-800">Manage Users & Roles</h2>
                    <p class="text-slate-500 text-xs md:text-sm">Change user roles and manage permissions</p>
                </div>
            </div>

            <!-- Search -->
            <div class="mb-4">
                <input wire:model.live.debounce.300ms="searchUsers" type="text" placeholder="Search users by name or email..." class="w-full md:w-64 border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
            </div>
                    ...
                    <div class="border border-slate-200 rounded-xl p-4 bg-white">
                        <div class="flex items-center mb-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="font-medium text-slate-800 truncate"><?php echo e($user->name); ?></p>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            ?>
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium <?php echo e($roleClasses); ?>">
                                <?php echo e(ucfirst($user->userRole->role ?? 'N/A')); ?>

                            </span>
                            <select wire:change="changeRole(<?php echo e($user->id); ?>, $event.target.value)" class="border border-slate-300 rounded-lg px-2 py-1.5 text-xs focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                                <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $roles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $role): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($role->id); ?>" <?php echo e($user->user_role_id == $role->id ? 'selected' : ''); ?>><?php echo e(ucfirst($role->role)); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><!--[if ENDBLOCK]><![endif]-->
                            </select>
        ...
        <div class="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6 mb-6">
            <div class="flex items-center mb-4 md:mb-6">
                <div class="w-8 h-8 md:w-10 md:h-10 bg-amber-100 rounded-lg md:rounded-xl flex items-center justify-center mr-3 md:mr-4">
                    <svg class="w-4 h-4 md:w-5 md:h-5 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/></svg>
                </div>
                <div>
                    <h2 class="text-lg md:text-xl font-bold text-slate-800">Manage Enrollments</h2>
                    <p class="text-slate-500 text-xs md:text-sm">View and remove student enrollments</p>
                </div>
            </div>

            <!-- Search -->
            <div class="mb-4">
                <input wire:model.live.debounce.300ms="searchEnrollments" type="text" placeholder="Search by student or module..." class="w-full md:w-64 border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-500 focus:border-transparent">
            </div>
                    ...
                    <div class="border border-slate-200 rounded-xl p-4 bg-white">
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex items-center">
                                ...
                                <span class="font-medium text-slate-800 text-sm"><?php echo e($enrollment->user->name); ?></span>
                            </div>
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium <?php echo e($enrollment->status === 'enrolled' ? 'bg-blue-100 text-blue-700' : 'bg-emerald-100 text-emerald-700'); ?>">
                                <?php echo e(ucfirst($enrollment->status)); ?>

                            </span>
                        </div>
                        <p class="text-sm text-slate-600 mb-3"><span class="font-medium">Module:</span> <?php echo e($enrollment->module->module); ?></p>
                        <button wire:click="removeStudentFromModule(<?php echo e($enrollment->id); ?>)" class="w-full inline-flex items-center justify-center px-3 py-2 rounded-lg text-sm font-medium bg-red-100 text-red-700 hover:bg-red-200 transition">
                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                            Remove
                        </button>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <div class="text-center py-8 text-slate-500">No enrollments found</div>
                <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
            </div>
        ...
        <div class="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6">
            <div class="flex items-center mb-4 md:mb-6">
                ...
                <div>
                    <h2 class="text-lg md:text-xl font-bold text-slate-800">Manage Teachers</h2>
                    <p class="text-slate-500 text-xs md:text-sm">Assign teachers to modules</p>
                </div>
            </div>

            <!-- Attach Teacher Form -->
            <form wire:submit="attachTeacherForm" class="mb-4 md:mb-6 p-3 md:p-4 bg-slate-50 rounded-xl">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
                    <div>
                        <label class="block text-xs md:text-sm font-medium text-slate-700 mb-2">Select Teacher</label>
                        <select wire:model="selectedTeacher" class="w-full border border-slate-300 rounded-lg px-3 md:px-4 py-2 md:py-2.5 text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                    ...
                    <div>
                        <label class="block text-xs md:text-sm font-medium text-slate-700 mb-2">Select Module</label>
                        <select wire:model="selectedModule" class="w-full border border-slate-300 rounded-lg px-3 md:px-4 py-2 md:py-2.5 text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                    ...
                    <div class="flex items-end">
                        <button type="submit" class="w-full bg-purple-600 text-white px-4 md:px-6 py-2 md:py-2.5 rounded-lg hover:bg-purple-700 transition font-medium text-sm">
                            Attach Teacher
                        </button>
                    </div>
                </div>
                ...
                <!--[if BLOCK]><![endif]--><?php $__empty_1 = true; $__currentLoopData = $teachers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $teacher): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="border border-slate-200 rounded-xl p-4 bg-white">
                        <div class="flex items-center mb-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="font-medium text-slate-800 truncate"><?php echo e($teacher->name); ?></p>
                                <p class="text-xs text-slate-500 truncate"><?php echo e($teacher->email); ?></p>
                            </div>
                        </div>
                        <div class="mb-3">
                            <p class="text-xs font-medium text-slate-600 mb-1">Assigned Modules:</p>
                            <div class="flex flex-wrap gap-1">
                                <!--[if BLOCK]><![endif]--><?php $__empty_2 = true; $__currentLoopData = $teacher->taughtModules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $module): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_2 = false; ?>
                                    <span class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-purple-100 text-purple-700"><?php echo e($module->module); ?></span>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_2): ?>
                                    <span class="text-slate-400 text-xs">None assigned</span>
                                <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                            </div>
                        </div>
                        <button wire:click="changeRole(<?php echo e($teacher->id); ?>, 3)" class="w-full inline-flex items-center justify-center px-3 py-2 rounded-lg text-sm font-medium bg-red-100 text-red-700 hover:bg-red-200 transition">
                            Remove Teacher
                        </button>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <div class="text-center py-8 text-slate-500">No teachers found</div>
                <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
            </div>
        </div>
    </div>
