<div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4 md:p-6">
    <div class="max-w-7xl mx-auto">
            ...
            <div class="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl md:text-3xl font-bold text-slate-800 mt-1">{{ $assignedModules->count() }}</p>
                    </div>
            ...
            <div class="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl md:text-3xl font-bold text-slate-800 mt-1">{{ $assignedModules->sum(fn($m) => $m->enrollments()->where('status', 'enrolled')->count()) }}</p>
                    </div>
            ...
            <div class="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6 col-span-2 md:col-span-1">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl md:text-3xl font-bold text-slate-800 mt-1">{{ $studentsInModule instanceof \Illuminate\Pagination\LengthAwarePaginator ? $studentsInModule->total() : $studentsInModule->count() }}</p>
                    </div>
            ...
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                    @foreach($assignedModules as $module)
                        <div class="border border-slate-200 rounded-xl p-4 md:p-5 hover:shadow-md transition cursor-pointer {{ $selectedModule == $module->id ? 'ring-2 ring-indigo-500 bg-indigo-50' : 'bg-white' }}" wire:click="$set('selectedModule', {{ $module->id }})">
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    <div class="flex items-center mb-2">
                                        ...
                                        <h3 class="font-semibold text-slate-800 text-sm md:text-base">{{ $module->module }}</h3>
                                    </div>
        ...
        <div class="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6">
            ...
            @if($studentsInModule instanceof \Illuminate\Pagination\LengthAwarePaginator ? $studentsInModule->isEmpty() : $studentsInModule->isEmpty())
                <div class="text-center py-8 md:py-12 bg-slate-50 rounded-xl">
            ...
            @else
                        ...
                        <div class="border border-slate-200 rounded-xl p-4 bg-white">
                            <div class="flex items-center mb-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="font-medium text-slate-800 truncate">{{ $enrollment->user->name }}</p>
                                    <p class="text-xs text-slate-500 truncate">{{ $enrollment->user->email }}</p>
                                </div>
                            </div>
                            <p class="text-xs text-slate-500 mb-3">Enrolled: {{ $enrollment->enrolled_at->format('M d, Y') }}</p>
                            <div class="flex space-x-2">
                                <button wire:click="gradeStudent({{ $enrollment->id }}, 'PASS')" class="flex-1 inline-flex items-center justify-center px-3 py-2 rounded-lg text-sm font-medium bg-emerald-100 text-emerald-700 hover:bg-emerald-200 transition">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                    Pass
                                </button>
                                <button wire:click="gradeStudent({{ $enrollment->id }}, 'FAIL')" class="flex-1 inline-flex items-center justify-center px-3 py-2 rounded-lg text-sm font-medium bg-red-100 text-red-700 hover:bg-red-200 transition">
                ...
                <!-- Pagination -->
                @if($studentsInModule instanceof \Illuminate\Pagination\LengthAwarePaginator)
                <div class="mt-4">
                    {{ $studentsInModule->links() }}
                </div>
                @endif
            @endif
        </div>
